...
/* Build a pay-to-pubkey (P2PK) output script from a raw public key.
 * Peercoin requires the coinbase's first output to be P2PK (rather than
 * P2PKH) so the node can extract a plain public key to validate the
 * block's vchBlockSig against. pubkey should be 33 bytes (compressed) or
 * 65 bytes (uncompressed). Returns the script length. */
int pubkey_to_txn(char *p2pk, const unsigned char *pubkey, const int pubkeylen)
{
	if (unlikely(pubkeylen != 33 && pubkeylen != 65)) {
		LOGERR("Invalid pubkey length %d passed to pubkey_to_txn", pubkeylen);
		return 0;
	}
	p2pk[0] = pubkeylen;           /* Push opcode: length is <= 75 so this is a direct push */
	memcpy(&p2pk[1], pubkey, pubkeylen);
	p2pk[1 + pubkeylen] = 0xac;    /* OP_CHECKSIG */
	return pubkeylen + 2;
}
