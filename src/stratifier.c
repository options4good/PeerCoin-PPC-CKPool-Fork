...
/* Signs the 32-byte internal-order block hash with the pool's block-signing
 * key (ckpool.blocksign_key, an EC_KEY* loaded at startup from the private
 * key matching the pubkey used in the coinbase's first output). Writes a
 * DER-encoded ECDSA signature into sig and its length into siglen. Returns
 * false if no key is loaded or signing fails. */
static bool blocksign_hash(const uchar *hash, unsigned char *sig, unsigned int *siglen)
{
	if (!ckpool.blocksign_key)
		return false;
		
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
	if (ECDSA_sign(0, hash, 32, sig, siglen, ckpool.blocksign_key) != 1)
		return false;
	      ...
	      const uchar *data, const uchar *hash, uchar *flip32, char *blockhash)
{
	...
	/* 5. Append vchBlockSig — required by Peercoin for both PoS and PoW
	 * blocks, signed by the private key behind the coinbase's first
	 * output pubkey. Without this the node's block deserializer will
	 * fail with "Block decode failed" since it always expects this
	 * trailing field. */
	if (!blocksign_hash(hash, sig, &siglen)) {
		LOGERR("Failed to sign block, submission will be rejected!");
	} else if (unlikely(siglen >= 0xfd)) {
		/* DER ECDSA sigs are always <= 72 bytes so this should never
		 * trigger, but guard the single-byte varint encoding below. */
		LOGERR("Block signature length %u too large to encode", siglen);
	} else {
		snprintf(sigvarint, sizeof(sigvarint), "%02x", siglen);
		strcat(gbt_block, sigvarint);
		__bin2hex(hexsig, sig, siglen);
		strcat(gbt_block, hexsig);
	}
...
void *stratifier(void *arg)
{
	...
	if (!ckpool.proxy) {
		...
		sdata->txnlen = pubkey_to_txn(sdata->txnbin, ckpool.blocksign_pubkey, ckpool.blocksign_pubkeylen);
